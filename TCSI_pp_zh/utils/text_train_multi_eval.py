...
# from loss.multi_focal_loss import KerasFocalLoss
# import os
# from focal_loss import BinaryFocalLoss
# from sklearn.metrics import precision_recall_curve
# from numpy.core.fromnumeric import argmax
# from loss.asymmetric_loss import AsymmetricLoss
# import torch.nn as nn
...
def train(config, model, train_iter, dev_iter, test_iter):
    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (trains_1, labels,lens,masks) in enumerate(train_iter):
            ...
            # pos_weight=torch.tensor([1,3,1,2,5,2,5,1,6,6,10]).to(config.device)pos_weight=pos_weight
            ...
            # criterion_weighted=focal_loss(alpha=config.focalloss_rate, gamma=2, num_classes=11)
            ...
            if total_batch % 500 == 0:
                ...
                # ture=true.clone().detach().cpu()
                predic=predic.clone().detach().cpu()
                # print(m(outputs.data))
                # print('predic.clone().detach().cpu()')
                # pdb.set_trace()
                # predic =
